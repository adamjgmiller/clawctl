...
async function getOpenClawVersion(ssh: SshClient): Promise<string> {
  const res = await ssh.exec('source ~/.nvm/nvm.sh 2>/dev/null; openclaw --version 2>/dev/null || echo unknown');
  return (res.stdout || 'unknown').trim();
}

async function updateAgent(ssh: SshClient, channel: string): Promise<{ stdout: string; stderr: string; code: number | null }> {
...
export function createUpdateCommand(): Command {
  ...
  update
    ...
    .action(async (opts: {
      ...
      console.log(chalk.bold(`Rolling update: ${agents.length} agent(s), channel=${opts.channel}, concurrency=${concurrency}`));
      if (opts.dryRun) {
      ...
      for (let i = 0; i < agents.length; i += concurrency) {
        ...
        const batchResults = await Promise.allSettled(batch.map(async (agent) => {
          ...
          if (!policy.allowed) {
            return { agentId: agent.id, agentName: agent.name, success: false, error: 'Denied by policy' } as UpdateResult;
          }
          ...
          try {
            ...
            if (updateRes.code !== 0) {
              const err = `Update failed (exit ${updateRes.code}): ${updateRes.stderr || updateRes.stdout}`.slice(0, 200);
              console.log(`  ${chalk.bold(agent.name)}: ${chalk.red('FAILED')} — ${err}`);
              return { agentId: agent.id, agentName: agent.name, success: false, previousVersion: prevVersion, error: err } as UpdateResult;
            }

            // Restart gateway if not --no-restart
            if (opts.restart !== false) {
              console.log(`  ${chalk.bold(agent.name)}: restarting gateway...`);
              await ssh.exec('systemctl --user restart openclaw-gateway.service 2>/dev/null || (source ~/.nvm/nvm.sh 2>/dev/null; openclaw gateway restart)');
              // Brief pause for gateway to come up
              await new Promise((r) => setTimeout(r, 3000));
            }

            const newVersion = await getOpenClawVersion(ssh);
            console.log(`  ${chalk.bold(agent.name)}: ${chalk.green('OK')} ${prevVersion} → ${newVersion}`);

            return { agentId: agent.id, agentName: agent.name, success: true, previousVersion: prevVersion, newVersion } as UpdateResult;
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            console.log(`  ${chalk.bold(agent.name)}: ${chalk.red('ERROR')} — ${msg}`);
            return { agentId: agent.id, agentName: agent.name, success: false, error: msg } as UpdateResult;
          } finally {
            ssh.disconnect();
          }
        }));
        ...
        for (const r of batchResults) {
          const result = r.status === 'fulfilled' ? r.value : { agentId: '?', agentName: '?', success: false, error: String(r.reason) } as UpdateResult;
          results.push(result);
          await audit('agent.update', {
            ...
            error: result.error,
            detail: { previousVersion: result.previousVersion, newVersion: result.newVersion, channel: opts.channel } as Record<string, unknown>,
          });
        }
      ...
      console.log(`  ${chalk.green(`${succeeded} succeeded`)}, ${failed > 0 ? chalk.red(`${failed} failed`) : '0 failed'}`);

      if (failed > 0) {
        await alert('warning', 'Rolling update had failures', `${failed} of ${results.length} agents failed to update to ${opts.channel}.`);
        process.exitCode = 1;
      } else {
        await alert('info', 'Rolling update complete', `${succeeded} agents updated to ${opts.channel}.`);
      }
    });
